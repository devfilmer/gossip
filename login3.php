...
<table>
	...
	<tr><td>User name</td><td><input type= "text" size="15" name="username" id="username"/></td></tr>
	<tr><td>Password</td><td><input type= "text" size="15" name="password" id ="password"/></td></tr>
...
if ($result) {
    ...
    while($row = mysql_fetch_array($result)) {
        ...
        if ($username == $name and $userpassword == $row[Password])
        {
		  ...
		  $url = 'http://spoiledpickle.com/sessiontest.php';
		  $fields = array('uname'=> $username, 'pwd' => urlencode($userpassword));
		  $fields_string = 'uname='.$username.'&'.'pwd='.$userpassword;
         ...
         // rtrim($fields_string, '&');
		  ...
		  echo "<script>alert('$fields_string')</script>";
	      echo "<script>alert('do it now')</script>";
