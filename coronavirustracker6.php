...
<head><script data-ad-client="ca-pub-9264799538995539" async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script><meta charset="UTF-8">
  ...
  <meta name="keywords" content="news,celebrities,politics,gossip,spoiled, pickle, coronavirus, map, tracker"><!-- Global site tag (gtag.js) - Google Analytics -->
  ...
  success: function(data) {
	...
	//alert(data.feed.description);
     //stuff += "<table>";
      ...
      for(var j=0; j< roughnews1.items.length; j++)
	{
		
		
		newstuff += "<a href = '";
		newstuff += roughnews1.items[j].link;
		newstuff += "'>";
           newstuff += roughnews1.items[j].title;
		newstuff += "</a>";
		newstuff += "<br>";
		newstuff += "-------------------";
		newstuff += "<br>";
...
$.ajax({
  ...
  success: function(data) {
      ...
      for(var j=0; j< data.items.length; j++)
	{
		...
		newstuff += "<a href = '";
		newstuff += data.items[j].link;
		newstuff += "'>";
           newstuff += data.items[j].title;
		newstuff += "</a>";
		newstuff += "<br>";
		newstuff += "-------------------";
		newstuff += "<br>";
...
$.ajax({
  ...
  success: function(data) {
      ...
      for(var j=0; j< data.items.length; j++)
	{
	    ...
	    thenewsfeed = document.getElementById("test2");
		thenewsfeed.innerHTML = "";
		thenewsfeed.innerHTML = newstuff;
...
function bolo2(huh){
	...
	var stuff2;
	 alert(roughnews);
      for(var j=0; j< roughnews.items.length; j++)
	{
		var str = roughnews.items[j].title;
		//alert(str);
		if (str.search(huh) != -1)
			{
				//alert("now what");
		...
		stuff2 += "<a href = '";
		stuff2 += roughnews.items[j].link;
		stuff2 += "'>";
           stuff2 += roughnews.items[j].title;
		stuff2 += "</a>";
		stuff2 += "<br>";
		stuff2 += "-------------------";
		stuff2 += "<br>";
