...
oscillatorAudioNode2.frequency.setValueAtTime(Math.pow(2, (timecount%difficulty) / 60), context2.currentTime);
}
...
boardpen.drawImage(presss, agamescreen.width/3, agamescreen.height/3, agamescreen.width/3, agamescreen.width/6);
...
function keyDownHandler(event) {
      ...
      event.keyCode == 82) && gameover == true) {
      ...
      event.keyCode == 83) && gameover == true) {
           //window.cancelAnimationFrame(startGame);
...
function invaderStatus()
{
    ...
    if (invadersdead == invaderArray.length) {resetInvaders();}
}
...
ctx.fillStyle = "red";
 //alert('i got here h');
    ctx.fillText("High Scores", highscorex-0.5*myhsfontheight, highscorey + 3*myhsfontheight);
    for (let r in myhighscores) {
      //alert('i got here');
      ctx.fillText(myhighscores[r].initials+": "+myhighscores[r].score, highscorex, 6*highscorey + r*myhsfontheight);
      //ctx.fillText("jjj", highscorex, highscorey + r * myhsfontheight);
    }
  
  ctx.fillText("Press 'S' to Play", highscorex-1.5*myhsfontheight, 6*highscorey + 12*myhsfontheight);
}
  ...
  pxhttp.onload = function() {
       ...
       //alert ("precheck for high score");
        if (checkedHigh==false){
          ...
          if(score > myhighscores[r].score){winner=true;break;}
          }          
          ...
          myhighscores = JSON.parse(this.responseText);
          precheck = true; checkedHigh = true; gameoverset= true;
          //window.requestAnimationFrame(gameLoop);
        //window.requestAnimationFrame(gameLoop);
...
function checkForHighScore(){
  ...
  xhttp.send("playerscore="+score+ "&"+"playerinitials="+person.toUpperCase());
...
boardpen.drawImage(gameoverpic,agamescreen.width*0.25, agamescreen.height*0.25, agamescreen.width*0.5, agamescreen.height/2);
...
ship = new Ship(agamescreen.width/3, agamescreen.height - agamescreen.width/20);
...
for (b=1; b< 5; b++)
{
  let abarricade = new Barricade(b*agamescreen.width/5, agamescreen.height-agamescreen.width/7);
...
//shift on right code
if ((invaderArray[i].x > agamescreen.width - invaderArray[i].width  || invaderArray[i].x < 0) && invaderArray[i].alive == true && invaderArray[i].crazy == false){
shift = true;
